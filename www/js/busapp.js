		...
		$(this).bus().loadCurrentEpisodesForProgram({"guid" : prgGuid, "size" : 1, "callback" : function(data) {
			if (data[0] != "No results found") {
				...
				$(rowElm).bus().loadNextEpisodeForProgram({"guid" : prgGuid, "size" : 1, "callback" : function(data) {
					var episode = data[0];
					var parsedDate = new Date(Date.parse(episode.airtime.start));
					var formattedDate = dateFormat(parsedDate, "ddd, mmm dS, yyyy, h:MM TT");
					$(rowElm).append("<td><i class='icon-arrow-right'></i>" + formattedDate + "/" + episode.airtime.start + "</td>");
				}});
			}
		}});
